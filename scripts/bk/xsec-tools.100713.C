...
void plotphi(int top, bool phi=kFALSE){
  ...
  while (key = (TKey*)nextkey()) {
    ...
    TString outdir = TString::Format("./polobs/top%d/%s/Varset1/theta", top, Q2Wdirname.Data());
    ...
    for (int i=0; i<nvarbins; i++){
      ...
      cout << "hname_pos = " << hname_pos << endl;
      cout << "hname_neg = " << hname_neg << endl;
      TH1D* hphi_unp = (TH1D*)fy->Get(hname_unp);
      TH1D* hphi_pos = (TH1D*)fy->Get(hname_pos);
      TH1D* hphi_neg = (TH1D*)fy->Get(hname_neg);
      if (hphi_unp==NULL || hphi_pos==NULL || hphi_neg==NULL) cout << "histogram not found" << endl;
      hphi_unp->SetLineColor(kBlue);
      hphi_pos->SetLineColor(kRed);
      hphi_neg->SetLineColor(kBlack);

      TH1D* hphi_unp_norm = (TH1D*)hphi_unp->Clone((TString)hphi_unp->GetName()+"_norm");
      hphi_unp_norm->Divide(&hnorm);
      TH1D* hphi_pos_norm = (TH1D*)hphi_pos->Clone((TString)hphi_pos->GetName()+"_norm");
      hphi_pos_norm->Divide(&hnorm);
      TH1D* hphi_neg_norm = (TH1D*)hphi_neg->Clone((TString)hphi_neg->GetName()+"_norm");
      hphi_neg_norm->Divide(&hnorm);

      TH1D* hphi_unp_norm_sphi = (TH1D*)hphi_unp_norm->Clone((TString)hphi_unp_norm->GetName()+"_sphi");
      hphi_unp_norm_sphi->Multiply(&hsinphi);
      TH1D* hphi_pos_norm_sphi = (TH1D*)hphi_pos_norm->Clone((TString)hphi_pos_norm->GetName()+"_sphi");
      hphi_pos_norm_sphi->Multiply(&hsinphi);
      TH1D* hphi_neg_norm_sphi = (TH1D*)hphi_neg_norm->Clone((TString)hphi_neg_norm->GetName()+"_sphi");
      hphi_neg_norm_sphi->Multiply(&hsinphi);
      ...
      integ = hphi_pos_norm_sphi->IntegralAndError(1, hphi_pos_norm_sphi->GetNbinsX(), integerr);
      hRvVar[POS].SetBinContent(i+1, integ/TMath::Pi());
      hRvVar[POS].SetBinError(i+1, integerr);
      integ = hphi_neg_norm_sphi->IntegralAndError(1, hphi_neg_norm_sphi->GetNbinsX(), integerr);
      hRvVar[NEG].SetBinContent(i+1, integ/TMath::Pi());
      hRvVar[NEG].SetBinError(i+1, integerr);

      //! Apply Method 1
      fphi->SetParameters(1,10,20,100);
      hphi_unp_norm->Fit(fphi,"");
      fphi->SetParameters(1,10,20,100);
      hphi_pos_norm->Fit(fphi,"");
      fphi->SetParameters(1,10,20,100);
      hphi_neg_norm->Fit(fphi,"");
      ...
      hphi_neg->Draw();
      ptneg->Draw();
      cphi.SaveAs(TString::Format("%s/%s.png", outdir.Data(), cphi.GetName()));
      cphi.Close();*/

      TCanvas cphi_norm = TCanvas(hphi_pos_norm->GetName(),hphi_pos_norm->GetName(),900, 600);
      ...
      /*TCanvas cphi_norm_sphi = TCanvas(hphi_pos_norm_sphi->GetName(),hphi_pos_norm_sphi->GetName(),900, 600);
      cphi_norm_sphi.Divide(3,1);
      cphi_norm_sphi.cd(1);
      hphi_unp_norm_sphi->Draw();
      ptunp->Draw();
      cphi_norm_sphi.cd(2);
      hphi_pos_norm_sphi->Draw();
      ptpos->Draw();
      cphi_norm_sphi.cd(3);
      hphi_neg_norm_sphi->Draw();
      ptneg->Draw();
      cphi_norm_sphi.SaveAs(TString::Format("%s/%s.png", outdir.Data(), cphi_norm_sphi.GetName()));
      cphi_norm_sphi.Close();*/
    }
