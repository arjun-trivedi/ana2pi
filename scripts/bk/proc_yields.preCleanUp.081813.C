	...
	for(Int_t iQ2bin = 0; iQ2bin < _user.nQ2bins; iQ2bin++){
		for(Int_t iWbin = 0; iWbin < _user.nWbins; iWbin++){
			...
			if (_useHel){
				...
				setCanvasStackNameTitle(POS);
	  			proc_hY5D(POS);
				proc_hY1D(POS);	

				setCanvasStackNameTitle(NEG);
				proc_hY5D(NEG);
				proc_hY1D(NEG);
...
void ProcYields::proc_hY5D(hel_t hel/*=UNPOL*/){
	...
	if (hel==UNPOL)	{
		...
		_dirObs = _dirQ2W->mkdir( TString::Format("hY5D_%s",helTitle[hel].Data()) );
	}		
	...
	for(Int_t iVarset=0;iVarset<nVarset;iVarset++){
		...
		//!RECO
		if(_issim || _isexp){
			if (hel==POS || hel==NEG) hY8D[iVarset][RECO]->GetAxis(H)->SetRange(hel,hel);
...
void ProcYields::proc_hY1D(hel_t hel/*=UNPOL*/){
	...
	for(Int_t iSeq=0;iSeq<nSeq;iSeq++){
		...
		if (_isexp && _useHel && seq==FULL) continue;
	...
	if (hel==UNPOL)	{
		if(_dirQ2W->GetDirectory("hY2D") == NULL) dirY2D = _dirQ2W->mkdir("hY2D");
		if(_dirQ2W->GetDirectory("hY1D") == NULL) dirY1D = _dirQ2W->mkdir("hY1D");
	}else if(hel==POS || hel==NEG){
		if(_dirQ2W->GetDirectory(TString::Format("hY2D_%s",helTitle[hel].Data())) == NULL) dirY2D = _dirQ2W->mkdir(TString::Format("hY2D_%s",helTitle[hel].Data()));
		if(_dirQ2W->GetDirectory(TString::Format("hY1D_%s",helTitle[hel].Data())) == NULL) dirY1D = _dirQ2W->mkdir(TString::Format("hY1D_%s",helTitle[hel].Data()));
	}
	for (Int_t iVarset=0;iVarset<nVarset;iVarset++){
		dirY2D->cd();
		...
		for(Int_t iVar=0; iVar<nVar; iVar++){
			...
			for(Int_t iSeq=0;iSeq<nSeq;iSeq++){
				...
				//! hY2D !//
				_dirQ2W->cd(TString::Format("%s/Varset%d", dirY2D->GetName(),iVarset+1));
				...
				//! hY1D !//
				_dirQ2W->cd(TString::Format("%s/Varset%d", dirY1D->GetName(),iVarset+1));
				...
				if(_useHel && var==PHI){
					if(hel==POS) {
						hY1D_phi_pos[iVarset][iSeq] = hY1D[iVarset][iVar][iSeq];
						hY1D_phi_pos[iVarset][iSeq]->Sumw2();
					}
					else if (hel==NEG) {
						hY1D_phi_neg[iVarset][iSeq] = hY1D[iVarset][iVar][iSeq];
						hY1D_phi_neg[iVarset][iSeq]->Sumw2();
					}
				} 
...
void ProcYields::proc_asym()
{
	...
	//! Create Canvas Managers !//
	cm_hAsym = new CanvasManager(_c_ww, _c_wh);
    //! Create Stacks for RECO, ACC_CORR !//
	for(Int_t iSeq=0;iSeq<nSeq;iSeq++){
		seq_t seq = static_cast<seq_t>(iSeq);
		if (seq==TH || seq==ACC || seq==HOLE || seq==FULL) continue;
		hs_hAsym[iSeq] = new THStack(*cname_hAsym[iSeq], *ctitle_hAsym[iSeq]);
	}
	...
	for (Int_t iVarset=0;iVarset<nVarset;iVarset++){
		...
		for(Int_t iVar=0; iVar<nVar; iVar++){
			...
			for(Int_t iSeq=0;iSeq<nSeq;iSeq++){
				...
				//! hAsym !//
				_dirQ2W->cd(TString::Format("%s/Varset%d", dirAsym->GetName(),iVarset+1));
				TH1D* hAsym = (TH1D*)hY1D_phi_pos[iVarset][iSeq]->Clone();
				hAsym->Sumw2();
				hAsym->Add(hY1D_phi_neg[iVarset][iSeq],-1);
			}
		}
	}
	Info("proc_asym", "done\n");
...
void ProcYields::setCanvasStackNameTitle(hel_t hel/*=UNPOL*/){
	...
	for (Int_t iSeq=0;iSeq<nSeq;iSeq++){
		...
		cname_hAsym[iSeq]  = new TString(tmp_name);
		ctitle_hAsym[iSeq] = new TString(tmp_title);
	}
}
