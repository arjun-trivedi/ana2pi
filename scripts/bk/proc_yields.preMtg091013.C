...
void ProcYields::Proc_hY5D(hel_t hel/*=UNPOL*/){
	...
	for(Int_t iVarset=0;iVarset<nVARSET;iVarset++){
		...
		if (hel==POS || hel==NEG){
			/*//test bug in THnBase::Add()
			THnSparse* h5a = (THnSparse*)_dirQ2W->Get("hY5D_POS/Varset1/hY5D_ACC_CORR");
			THnSparse* h5b = (THnSparse*)_dirQ2W->Get("hY5D_POS/Varset1/hY5D_RECO");
			//h5a->Add(h5b,-1);
			h5a->Add(h5b,1);
			TH1D* h1 = (TH1D*)h5a->Projection(PHI,"E");
			h1->SetNameTitle("ha","hb");*/
