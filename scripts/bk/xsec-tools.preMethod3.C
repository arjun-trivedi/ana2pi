...
void plotphi(int top,bool sim=kFALSE){
  ...
  while (key = (TKey*)nextkey()) {
    ...
    for (int i=0; i<nvarbins; i++){
      ...
      hRvVar[UNPOL].SetBinError(i+1, integerr);
      if (exp){
      ...
      /*TCanvas cphi = TCanvas(hphi[UNPOL]->GetName(),hphi[UNPOL]->GetName(),900, 600);
      cphi.Divide(3,1);
      cphi.cd(1);
      hphi_unp->Draw();
      pt[UNPOL]->Draw();
      ...
      /*TCanvas cphi_norm_sphi = TCanvas(hphi_norm_sphi[UNPOL]->GetName(),hphi_norm_sphi[UNPOL]->GetName(),900, 600);
      cphi_norm_sphi.Divide(3,1);
      cphi_norm_sphi.cd(1);
      hphi_norm_sphi[UNPOL]->Draw();
      pt[UNPOL]->Draw();
      cphi_norm_sphi.cd(2);
      hphi_norm_sphi[POS]->Draw();
      pt[POS]->Draw();
      cphi_norm_sphi.cd(3);
      hphi_norm_sphi[NEG]->Draw();
      pt[NEG]->Draw();
