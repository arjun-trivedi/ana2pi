...
void plotr(int top,bool sim=kFALSE){
  ...
  while (key = (TKey*)nextkey()) {
    ...
    if (hY5D[ACC_CORR][NEG]==NULL) cout << "NULL!" << endl;

    //! NORMALIZE hY5D[FULL][POS]
    //hY5D[FULL][POS]->Scale(1/norm);

    //! MULTIPLY hY5D[FULL][POS] by sin(phi)
    ...
    //! MULTIPLY hY5D[ACC_CORR][NEG] by sin(phi)
