...
void ProcYields::Proc_hY5D(hel_t hel/*=UNPOL*/){
	...
	for(Int_t iVarset=0;iVarset<nVARSET;iVarset++){
		...
		if(_issim || _isexp){
			...
			setHistNameTitleAxes_hY5D(_hY5D[iVarset][ACC_CORR], iVarset, ACC_CORR);
		...
		if(_issim){
			...
			setHistNameTitleAxes_hY5D(_hY5D[iVarset][HOLE], iVarset, HOLE);
		...
		if (_issim || _isexp){
			...
			setHistNameTitleAxes_hY5D(_hY5D[iVarset][FULL], iVarset, FULL);
	...
	for (Int_t iVarset=0;iVarset<nVARSET;iVarset++){
		...
		for(Int_t iVar=0; iVar<nVAR; iVar++){
			...
			for(Int_t iSeq=0;iSeq<nSEQ;iSeq++){
				...
				if(_useHel && var==PHI){
					if(hel==POS) {
						_hY1D_phi_pos[iVarset][iSeq] = _hY1D[iVarset][iVar][iSeq];
						_hY1D_phi_pos[iVarset][iSeq]->Sumw2();
					}
					else if (hel==NEG) {
						_hY1D_phi_neg[iVarset][iSeq] = _hY1D[iVarset][iVar][iSeq];
						_hY1D_phi_neg[iVarset][iSeq]->Sumw2();
					}
				} 
...
void ProcYields::Proc_asym()
{
	...
	//! Create Canvas Managers !//
	_cm_hAsym = new CanvasManager(_c_ww, _c_wh);
    //! Create Stacks for RECO, ACC_CORR !//
	_hs_hAsym[RECO] = new THStack(*_cname_hAsym[RECO], *_ctitle_hAsym[RECO]);
	_hs_hAsym[ACC_CORR] = new THStack(*_cname_hAsym[ACC_CORR], *_ctitle_hAsym[ACC_CORR]);
	...
	for (Int_t iVarset=0;iVarset<nVARSET;iVarset++){//!VARSET loop!//
		...
		TH1D* hAsym_reco = (TH1D*)_hY1D_phi_pos[iVarset][RECO]->Clone();
		TString name  = _hY1D_phi_pos[iVarset][RECO]->GetName();
		name.ReplaceAll("hY1D","hAsym");
		hAsym_reco->SetName(name);
		hAsym_reco->Sumw2();
		hAsym_reco->Add(_hY1D_phi_neg[iVarset][RECO],-1);

		TH1D* hAsym_acc_corr = (TH1D*)_hY1D_phi_pos[iVarset][ACC_CORR]->Clone();
		name  = _hY1D_phi_pos[iVarset][ACC_CORR]->GetName();
		name.ReplaceAll("hY1D","hAsym");
		hAsym_acc_corr->SetName(name);
		hAsym_acc_corr->Sumw2();
		hAsym_acc_corr->Add(_hY1D_phi_neg[iVarset][ACC_CORR],-1);
	}
	Info("Proc_asym", "done\n");
}
