...
inline bool fidu_e_sub(float pel,float thetael,float phiel,bool in_fid_reg){
  return fidu_e_sub_(&pel,&thetael,&phiel,&in_fid_reg);
}
inline bool hadronfid(bool result,float theta,float phi,int s){
...
/* Returns true if electron is within fiducial volume
  + 'id' is unused
*/
inline bool Fiducial_e16_elctrn(int id, float p, float theta, float phi)
{
  bool in_fid_reg=false;
  return fidu_e_sub(p,theta,phi,in_fid_reg);
}

inline bool Fiducial_e16_hdrn(float theta, float phi,int sector)
{
  bool in_fid_reg=false;
