...
def plot_intg_acc(obsdir,simnum,q2binl,vst="VST1"):
	'''
	...
	FIN=ROOT.TFile(os.path.join(DATADIR,simnum,'yield_sim.root'))
	#OUTDIR=os.path.join(DATADIR,simnum,'intg-eff_top%s'%''.join(str(t) for t in tops))
	OUTDIR=os.path.join(DATADIR,simnum,'intg-eff')
...
def plot_acc_q2wbin(sim,vst=1,q2wbin="1.25-1.75_1.575-1.600",crs_w_bin=3):
	'''
	...
	#! --Q2 bin range
	q2bin_le=h8R.GetAxis(h7_DIM['Q2']).FindBin(q2min+0.5/2)
	q2bin_ue=h8R.GetAxis(h7_DIM['Q2']).FindBin(q2max-0.5/2)
	h8R.GetAxis(h7_DIM['Q2']).SetRange(q2bin_le,q2bin_ue)
	q2bin_le=h8T.GetAxis(h7_DIM['Q2']).FindBin(q2min+0.5/2)
	q2bin_ue=h8T.GetAxis(h7_DIM['Q2']).FindBin(q2max-0.5/2)
	h8T.GetAxis(h7_DIM['Q2']).SetRange(q2bin_le,q2bin_ue)
	#!-- W bin range
	wbin_le=h8R.GetAxis(h7_DIM['W']).FindBin(wmin+0.025/2)
	wbin_ue=h8R.GetAxis(h7_DIM['W']).FindBin(wmax-0.025/2)
	h8R.GetAxis(h7_DIM['W']).SetRange(wbin_le,wbin_ue)
	wbin_le=h8T.GetAxis(h7_DIM['W']).FindBin(wmin+0.025/2)
	wbin_ue=h8T.GetAxis(h7_DIM['W']).FindBin(wmax-0.025/2)
	h8T.GetAxis(h7_DIM['W']).SetRange(wbin_le,wbin_ue)
	cq2w=ROOT.TCanvas("Q2,W")
	cq2w.Divide(2,1)
	cq2w.cd(1)
	hq2wT=h8T.Projection(h7_DIM['Q2'],h7_DIM['W'],"E")
	hq2wT.Draw("colz")
	cq2w.cd(2)
	hq2wR=h8R.Projection(h7_DIM['Q2'],h7_DIM['W'],"E")
	hq2wR.Draw("colz")
	...
	if not ROOT.gROOT.IsBatch():
    		...
    		wait(True)
def plot_acc_q2wbin_5D(obsdir,q2wbin="1.75-2.25_1.575-1.600",simnum='siml',vst=1):
	'''
	...
	#! Get 5D histograms
	h5T=FIN.Get('%s/VST%d/T/h5'%(q2wbin,vst))
	h5R=FIN.Get('%s/VST%d/R/h5'%(q2wbin,vst))
	h5A=FIN.Get('%s/VST%d/A/h5'%(q2wbin,vst))
