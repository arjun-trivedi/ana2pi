...
class DispYieldsLite:
	def __init__(self,obsdir,simnum='siml'):#,tops=[1,2,3,4]):
		...
		self.FEXP=root_open(os.path.join(self.DATADIR,self.SIM_NUM,'yield_exp.root'))
		...
		self.FSIM=root_open(os.path.join(self.DATADIR,self.SIM_NUM,'yield_sim.root'))
	...
	def extract_and_plot_R2(self,h5d,R2l,mthd,viewl,plotphiproj,dtypl,seql):#,hel,dtypl,seql):
		...
		"""
		print "In DispYieldsLite::extract_obs_R2()"
	...
	def plot_obs_R2(self,hR2d,R2,dtypl,seql):
		print "In DispYieldsLite::plot_obs_R2()"
			...
			for var in VARS:
				...
				for hel in hR2d.keys():
					...
					for q2bin_le in q2bins_lel:
						...
						c.Close()
		print "Done DispYieldsLite::plot_obs_R2()"
	...
	def plot_obs_R2_view2(self,hR2d,R2,dtypl,seql):
		...
		print "In DispYieldsLite::plot_obs_R2_view2()"
		...
		for wbin in wbins_le:
			for iq2bin,q2bin in enumerate(q2bins_le):
				for hel in hR2d:
					...
					c.Close()
		print "Done DispYieldsLite::plot_obs_R2_view2()"
		return
	...
	def extract_R2_from_phiproj(self,h5d,fpard,R2):
		...
		"""
		print "In DispYieldsLite::extract_R2_from_phiproj for R2=%s"%(R2)
	...
	def plot_phiproj(self,hphiprojd,fpard,dtypl,seql):
		...
		+ plot phiprojs
		"""
		print "In DispYieldsLite::plot_phiproj"
		...
		print "Done DispYieldsLite::plot_phiproj()"	
	...
	def disp_1D(self,view,q2min=1.25,q2max=5.25,wmin=1.400,wmax=2.125,norm=False,dbg=False,dtypl=['EXP','SIM'],seql=['T','R','C','H','F']):
		...
		print "In DispYieldsLite::disp_1D()"
		...
		if view=="q2_evltn":
			...
			sys.exit("view=%s not recognized. Exiting."%view)
		print "Done DispYieldsLite::disp_1D()"
	...
	def disp_obs_R2(self,R2l,mthd,viewl,plotphiproj,q2min,q2max,wmin,wmax,dtypl,seql):
		...
		"""
		print "In DispYieldsLite::disp_obs_R2()"
		...
		print "Done DispYieldsLite::disp_obs_R2()"
		print "If the progam is not terminating, then Python is probably doing \"garbage collection\"(?); Wait a while!"
		return
	...
	def disp_integ_yield(self,seql=['C','F'],vst='VST1',norm=False):
		...
		for seq in seql:
			for q2wbin in q2wbinl:
				...
				h5=self.FEXP.Get("%s/%s/%s/h5"%(q2wbin,vst,seq))
				y[seq,q2bin][w]=thntool.GetIntegral(h5)
				if norm==True:
	...
	def get_xbng(self,x,q2wbinl):
		...
		for q2wbin in q2wbinl:
			...
			if(x=='Q2'):
				...
				sys.exit("DispYieldsLite::get_xbng() x=%s not recognized"%x)
