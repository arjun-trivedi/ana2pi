...
def plot_acc_q2wbin_5D(obsdir,q2wbin="1.75-2.25_1.575-1.600",simnum='siml',vst=1):
	...
	FIN=ROOT.TFile(os.path.join( DATADIR,simnum,"yield.root"))
	print "FIN=",FIN.GetName()

	#! Get 5D histograms
	h5T=FIN.Get('%s/ST/VST%d/h5'%(q2wbin,vst))
	h5R=FIN.Get('%s/SR/VST%d/h5'%(q2wbin,vst))
	h5A=FIN.Get('%s/SA/VST%d/h5'%(q2wbin,vst))
