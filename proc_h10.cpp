...
EpProcessor* SetupProcs(){
   ...
   if (procorder_tokens->IsEmpty()) {
      ...
      while(TObjString *obj_str = (TObjString*)iter.Next()) {
         ...
         else if (str.EqualTo("delastR"))            proc = new ProcDelast(mkdir("delast"),dH10,dAna,kFALSE,kTRUE);
         else if (str.EqualTo("delastR_tree"))       proc = new ProcDelast(mkdir("delast"),dH10,dAna,kFALSE,kTRUE,kTRUE);
