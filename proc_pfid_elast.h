#ifndef PROCPFIDELAST_H
#define PROCPFIDELAST_H
...
/*
 [06-28-15]
 + Processor must follow ProcSkimQElast:ProcPidElast

 + Pass Events only if:
	+ dAna->pid_elast.h10IdxP>0 && each of p is in fiducial region of detector
*/
class ProcPFidElast : public EpProcessor {

public:
	ProcPFidElast(TDirectory *td,DataH10* dataH10,DataAna* dataAna, 
...
ProcPFidElast::ProcPFidElast(TDirectory *td,DataH10* dataH10,DataAna* dataAna, 
                   Bool_t monitor/* = kFALSE*/,Bool_t monitorOnly /*= kFALSE*/)
...
ProcPFidElast::~ProcPFidElast()
{
	
}

void ProcPFidElast::handle()
{
	//Info("ProcPFidElast::handle()", "");
	pass = kFALSE;
	hevtsum->Fill(EVT);
	...
	updatePFid();
	dAna->fillHistsPFidElast(hists[MONMODE][EVTINC]);
	...
	if (in_fid_rgn)
	{
		...
		dAna->fillHistsPFidElast(hists[CUTMODE][EVTINC]);
