...
void SelH10::Init(TTree *tree) {
	...
	isSim = kTRUE;
	TBranch *testbranch = fChain->GetBranch("rf_time2"); //e1f
	...
	if (isExp) {
		Info("Init", "binding e1f-specific branches");
		fChain->SetBranchAddress("evthel", &data.h10.evthel, &b_evthel); //e1f
	...
	//if (fChain->GetBranch("l2bit")) {
		...
		//these branches are named the same but have different data types than e1f h10 tree
