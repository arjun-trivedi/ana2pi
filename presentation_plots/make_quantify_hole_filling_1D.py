...
+ In this script the effect of hole-filling is demonstrated with the aid 
  of Obs_1D for Q2WBINS=['2.40-3.00_1.725-1.750','4.20-5.00_2.100-2.125']
...
fobs_lQ2    =ROOT.TFile("%s/thesis_obs_norm_ST_shape_122817/lowQ2/Obs_1D/Obs_1D.root"%os.environ['OBSDIR_E162'],"READ")
fobs_hQ2    =ROOT.TFile("%s/thesis_obs_norm_ST_shape_122817/highQ2/Obs_1D/Obs_1D.root"%os.environ['OBSDIR_E162'],"READ")
...
for q2wbin in Q2WBINS:#'2.40-3.00_1.725-1.750'
	...
	for pm in PAD_MAP['1D']:
		...
		hobs['EC',vst,var]=fobs.Get("%s/h1_%s_%d_%s"%(q2wbin,'EC',vst,var))
		hobs['EF',vst,var]=fobs.Get("%s/h1_%s_%d_%s"%(q2wbin,'EF',vst,var))
	...
	for item in PAD_MAP['1D']:
		...
		if pad==1:
			...
			l.Draw()
	c.SaveAs("%s/HF_EC_EF_1D_%s.png"%(OUTDIR,q2wbin))
	...
	# tool.wait()
	#! 2. .png->.pdf
	cmd=['convert',"%s/HF_EC_EF_1D_%s.png"%(OUTDIR,q2wbin),"%s/HF_EC_EF_1D_%s.pdf"%(OUTDIR,q2wbin)]
	tool=subprocess.Popen(cmd,stderr=subprocess.STDOUT)
	tool.wait()

#! Now make plot of the relative systematic error
hrelse=fsyserrs.Get("SYST_ERR_HF_REL_1D")
hrelse.SetTitle("#splitline{Distribution of Relative Systematic Uncertainty (%) to}{Single Differential Cross-Sections from Hole Filling}")
...
hrelse.Draw("hist")
c1.Update()
c1.SaveAs("%s/rel_syst_err_HF_1D.png"%(OUTDIR))
c1.SaveAs("%s/rel_syst_err_HF_1D.pdf"%(OUTDIR))
