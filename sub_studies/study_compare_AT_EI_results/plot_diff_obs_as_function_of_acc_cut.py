...
#! imports from disp_obs.py
from disp_obs import VAR_NAMES_PLAIN, R2S, R2_NAMED

import numpy as np

'''
+ Idea is to use this script to plot the difference between two sets of observables as a function of acceptance cut
+ Started as a copy of plot_diff_obs.py used for ananote

>plot_diff_obs_as_function_of_acc_cut.py obsdir1 obsdir2 expctd_diff expctd_diff_tlrnc dbg[=False]
...
# #print "path=",path
OUTDIR="%s/diff_obs/"%(OBSDIR[OBS2])
#print "OUTDIR=",OUTDIR
#sys.exit()
if DBG:
	OUTDIR="%s/diff_obs_dbg"%(OBSDIR[OBS2])
if not os.path.exists(OUTDIR):
	os.makedirs(OUTDIR)
print "OUTDIR=",OUTDIR
#sys.exit()

#! Create FIN(obsn,obst)
#! + obsn=OBS1,OBS2
#! + obst=1D,itg
FIN=OrderedDict()
for item in itertools.product([OBS1,OBS2],['Obs_1D','Obs_itg']):
	obsn,obst=item[0],item[1]
	if obst=="Obs_1D":
		dirname="Obs_1D_norm"
		fname="obs_1D.root"
	elif obst=="Obs_itg":
		dirname="Obs_Itg_Yld_norm"
		fname="obs_itg_yld.root"
	FIN[(obsn,obst)]="%s/%s/%s"%(OBSDIR[obsn],dirname,fname)
print "Number of files in FIN=(should be 2)",len(FIN)
print "*** FIN pretty print: ***"
for k in FIN:
	obsn,obst=k[0],k[1]
	print "FIN(%d,%s)=%s"%(obsn,obst,FIN[(obsn,obst)])
print "******"
#sys.exit()
...
def proc_Obs_1D():
	print "*** In proc_Obs_1D() ***\n"

	#! create structure to contain:
	#! + hobs[obsn]=f(q2wb,seq,vst,var)
	#! + hdiff     =f(q2wb,seq,vst,var)
	#! hobs
	hobs=OrderedDict()
	...
	#! nbins=f(q2wb,seq,vst,var)
	#! + Keep track of total bins processed
	nbins=OrderedDict()
	...
	fout=open("%s/1D.txt"%(OUTDIR_1D),'w')
	#! Descriptive opening remark in fout
	fout.write("*** This file will note bins where absolute diff in %s is greater than stat errs ***\n"%(obst))
	fout.write("\n")

	#! Now begin to fill this structure
	#! open f[obsn]
	f=[0 for i in range(NOBS)]
	f[OBS1]=root_open(FIN[(OBS1,'Obs_1D')])
	f[OBS2]=root_open(FIN[(OBS2,'Obs_1D')])
	#print f
	...
	for iq2wbin,q2wbin in enumerate(q2wbinl):
		...
		#! i. Get from f[obsn], hobs[obsn][q2wb,seq,vst,var]
		for iseq,seq in enumerate(['EC','EF']):
			for pm in PAD_MAP:
				vst,var=pm[1],pm[2]
				hobs[OBS1][q2wbin,seq,vst,var]=f[OBS1].Get("%s/h1_%s_%d_%s"%(q2wbin,seq,vst,var))
				hobs[OBS2][q2wbin,seq,vst,var]=f[OBS2].Get("%s/h1_%s_%d_%s"%(q2wbin,seq,vst,var))
				...
				hdiff[q2wbin,seq,vst,var]=ROOT.TH1F(hname,htitle,NBINS,XMIN,XMAX)
				hdiff_gt_staterr[q2wbin,seq,vst,var]=ROOT.TH1F("%s_gt_staterr_"%hname,"%s_gt_staterr"%htitle,NBINS,XMIN,XMAX)
				#! loop over bins of hobs1 and fill ((hobs1-hobs2)/hobs1)*100
				nbins=hobs[OBS1][q2wbin,seq,vst,var].GetNbinsX()
				...
				for ibin in range(nbins):
					obs1=hobs[OBS1][q2wbin,seq,vst,var].GetBinContent(ibin+1)
					obs2=hobs[OBS2][q2wbin,seq,vst,var].GetBinContent(ibin+1)
					stat_err1=hobs[OBS1][q2wbin,seq,vst,var].GetBinError(ibin+1)
					stat_err2=hobs[OBS2][q2wbin,seq,vst,var].GetBinError(ibin+1)
					diff=calc_rel_diff(obs1,obs2)
					abs_diff=math.fabs(obs2-obs1)
					#! Fill hdiff
					hdiff[q2wbin,seq,vst,var].Fill(diff)
					...
					if abs_diff_gt_staterr:
						...
						if hlindiff_drawn==False:
							hlindiff[q2wbin,seq,vst,var]=get_hlindiff(hobs[OBS1][q2wbin,seq,vst,var],hobs[OBS2][q2wbin,seq,vst,var])
							# if var=='M1' or var=='M2':
							# 	wmax=get_wbin_ue(q2wbin)
							# 	setM1M2axisrange(hlindiff[q2wbin,seq,vst,var],vst,var,wmax)
							hlindiff_drawn=True
						#! 2. Fill hdiff_gt_staterr
						hdiff_gt_staterr[q2wbin,seq,vst,var].Fill(diff)
						#! Write info to file
						#! note binn and binle
						binn=ibin+1
						binle=hobs[OBS1][q2wbin,seq,vst,var].GetBinLowEdge(ibin+1)
						#! 3. Now write to to file
						# fout.write("diff=%.2f for q2wb,seq,vst,var,var_name,binn,binle=%s,%s,%d,%s,%s,%d,%.3f\n"
						# 	            %(diff,q2wbin,seq,vst,var,VAR_NAMES_PLAIN[(vst,var)],binn,binle))
						fout.write("*** diff(%), q2wb, seq, vst, var, var_name, binn, binle, obs1+/3*stat_err1, obs2+/-3*stat_err2:***\n")
						fout.write("%.2f, %s, %s, %d, %s, %s, %d, %.3f, %f+/-%f, %f+/-%f\n"%(diff,q2wbin,seq,vst,var,VAR_NAMES_PLAIN[(vst,var)],
						...
						if any([math.fabs(diff)>x for x in DIFF_THRSHLD_VALS]):
							...
							fout.write("\n")
				#! Fill nbins
				nbins[q2wbin,seq,vst,var]=hobs[OBS1][q2wbin,seq,vst,var].GetNbinsX()
	...
	# 	nq2wbins=5*29
	# nhobs=len(hobs[OBS1,'Obs_1D'])+len(hobs[OBS2,'Obs_1D'])
	...
	#! hlindiff: Obs_1D
	#! Get list of q2wbins from hlindiff
	q2wbinl_1D=[k[0] for k in hlindiff]
	...
	for q2wbin in q2wbinl_1D:
		...
		tl=OrderedDict()
		#! Now loop over hlindiff and plot only where q2wbin1=q2wbin
		for k in hlindiff:
			...
			for item in PAD_MAP:
				...
				if vst_pm==vst and var_pm==var:
					...
					if seq=='EF' or seq=='EC':
						ymin=-50 #hlindiff[k].GetMinimum()
						ymax=50 #hlindiff[k].GetMaximum()
						...
						if crntpad2==None:
							print "Null pointer at: proc_Obs_1D: Plot hlindiff 1D: crntpad2=pad_p2.cd(pad)\n"
	...
	for seq in ['EC','EF']:
		...
		for k in hdiff:
			...
			if i==0: #! then make clone
				...
				hdiff_itg_gt_staterr=hdiff_gt_staterr[k].Clone("hdiff_itg_%s"%seq)
		...
		l.SetBorderSize(0)
		l.SetTextSize(0.04)
		n_tot_l=[nbins[k] if k[1]==seq else 0 for k in nbins.keys()]
		n_tot=sum(n_tot_l)
		prcnt_itg=(n_itg/ntot)*100
		n_itg=hdiff_itg.GetEntries()
		n_itg_gt_staterr=hdiff_itg_gt_staterr.GetEntries()
...
def proc_Obs_itg():
	...
	#! open f[obsn]
	f=[0 for i in range(NOBS)]
	f[OBS1]=root_open(FIN[(OBS1,'Obs_itg')])
	f[OBS2]=root_open(FIN[(OBS2,'Obs_itg')])
	...
	for iq2wbin,q2wbin in enumerate(q2wbinl):
		...
		for seq in ['EC','EF']:
			hobs[OBS1][q2wbin,seq]=f[OBS1].Get("%s/hW_%s_1_THETA"%(q2wbin,seq))
			hobs[OBS2][q2wbin,seq]=f[OBS2].Get("%s/hW_%s_1_THETA"%(q2wbin,seq))
			#! Create and fill hdiff
			...
			for ibin in range(nbins):
				...
				if abs_diff_gt_staterr:
					...
					# 	fout.write("diff is greater than 3*stat_err1\n")
					# if math.fabs(diff)>3*stat_err2:
					# 	fout.write("diff is greater than 3*stat_err2\n")
					#! check if diff greater than values in DIFF_THRSHLD_VALS
					if any([math.fabs(diff)>x for x in DIFF_THRSHLD_VALS]):
						thrshld_val=get_closest_diff_thrshld_val(diff)
						fout.write("diff exceeded-threshold-%d\n"%thrshld_val)
					fout.write("\n")
			#! Fill nbins
			nbins[q2wbin,seq]=hobs[OBS1][q2wbin,seq].GetNbinsX()
