...
#! imports from proc_h8.py and disp_obs.py
...
'''
[11-20-17] 
+ Started as a copy of elast_lite/obs_2pi/plot_diff_obs.py
+ Quick and Dirty (even the follow docs may need to be updated)
...
>plot_diff_obs.py obsdir1 obsdir2 outdir expctd_diff expctd_diff_tlrnc dbg[=False]
...
'''
USAGE=">plot_diff_obs.py obsdir1 obsdir2 outdir expctd_diff expctd_diff_tlrnc dbg[=False]"
...
OUTDIR=sys.argv[3]
EXPCTD_DIFF=float(sys.argv[4])
EXPCTD_DIFF_TLRNC=float(sys.argv[5])

if len(sys.argv)>6: #! User input dbg, else use dbg=False
	if sys.argv[6]=="False":
		DBG=False
...
#! + obsn=OBS1,OBS2
#! + obst=1D,R2,itg
#! Define OBST: Quick and Dirty
OBST=[['Obs_1D','Obs_1D_norm/obs_1D.root'],['Obs_itg','Obs_Itg_Yld_norm/obs_itg_yld.root'],['Obs_R2','Obs_R2_EC_EF_ST/mthd_phi-proj-fit_NQ/obs_R2.root']]
FIN=OrderedDict()
for item in itertools.product([OBS1,OBS2],OBST):
	obsn,obst,froot=item[0],item[1][0],item[1][1]
	FIN[(obsn,obst)]="%s/%s"%(OBSDIR[obsn],froot)
...
def proc_Obs_1D_and_R2():
	...
	for obst in  ['Obs_1D','Obs_R2']:
		...
		f[OBS1]=root_open(FIN[(OBS1,obst)])
		f[OBS2]=root_open(FIN[(OBS2,obst)])
