...
'''
+ Idea is to use this script to plot the difference between two sets of observables extracted with different SA-rel-err-cut
...
'''
USAGE=">plot_diff_obs.py obsdir1-lowQ2 obsdir1-highQ2 obsdir2-lowQ2 obsdir2-highQ2 expctd_diff expctd_diff_tlrnc output_tag dbg[=False]"
...
OBSDIR[OBS1]={'lowQ2':sys.argv[1],'highQ2':sys.argv[2]}
OBSDIR[OBS2]={'lowQ2':sys.argv[3],'highQ2':sys.argv[4]}
EXPCTD_DIFF=float(sys.argv[5])
EXPCTD_DIFF_TLRNC=float(sys.argv[6])
OUTPUT_TAG=sys.argv[7]
...
#! put output in $STUDY_OBS_AS_FUNCTION_OF_SA_REL_ERR_CUT
basepath=os.environ['STUDY_OBS_AS_FUNCTION_OF_SA_REL_ERR_CUT']
OUTDIR="%s/results_%s/%s"%(basepath,DATE,OUTPUT_TAG)# obsnamediff)
if DBG:
	OUTDIR="%s/dbg/results_%s/%s"%(basepath,DATE,OUTPUT_TAG)# obsnamediff)
...
#! + q2=lowQ2,highQ2
#! + obst=1D,R2,itg
FPATH={'Obs_1D':'Obs_1D_norm/obs_1D.root', 'Obs_R2':'Obs_R2_EC_EF_ST/mthd_phi-proj-fit_NQ/obs_R2.root', 'Obs_itg':'Obs_Itg_Yld_norm/obs_itg_yld.root'}
FIN=OrderedDict()
for item in itertools.product([OBS1,OBS2],['lowQ2','highQ2'],['Obs_1D','Obs_R2','Obs_itg']):
	obsn,q2,obst=item[0],item[1],item[2]
	FIN[(obsn,q2,obst)]="%s/%s"%(OBSDIR[obsn][q2],FPATH[obst])
